...
export const ActiveTeam = () => {
  const { activeTeam, startGettingMembers, isLoading, userTeams } = useDashboard();
  const firstLogging = parseInt(localStorage.getItem("firstLoggin") || "0", 10);

  useEffect(() => {
    if (activeTeam) startGettingMembers(activeTeam._id);
  }, [activeTeam, startGettingMembers]);
        ...
        {!activeTeam ? (
          <div className='flex flex-col justify-center mx-auto my-0 pb-20 space-y-2 duration-700'>
            <div className='text-4xl font-poppins text-primary/80'>
              {!isLoading ? (
                <span className='dark:text-tertiary text-primary pb-44 sm:text-lg md:text-xl lg:text-2xl'>Selecciona un equipo.</span>
              ) : (
